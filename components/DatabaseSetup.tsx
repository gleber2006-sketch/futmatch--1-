...
create policy "Users can see messages they sent or received." on direct_messages for select using (auth.uid() = sender_id or auth.uid() = receiver_id);
create policy "Users can send direct messages." on direct_messages for insert with check (auth.uid() = sender_id);
create policy "Users can update received direct messages." on direct_messages for update using (auth.uid() = receiver_id);
