        ...
        <div 
            ...
            style={{ 
                backgroundImage: formData.bannerUrl 
                    ...
                    : 'linear-gradient(to right, #064e3b, #111827)' 
            }}
            onClick={handleBannerClick}
        >
            <div className={`absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/20 to-transparent ${!formData.bannerUrl ? 'bg-black/20' : ''}`}></div>
            
            {!isEditing && (
                <div className="absolute bottom-0 left-0 right-0 pb-8 flex flex-col items-center justify-end z-10 animate-fade-in">
                     <div className="w-36 h-36 rounded-full border-4 border-white/10 shadow-2xl mb-4 relative overflow-hidden">
                         <img
                     ...
                     <p className="text-gray-200 text-sm flex items-center justify-center gap-1 mb-3 drop-shadow-md">
                        📍 {user.city || 'Cidade não informada'}, {user.state || 'SP'}
                     </p>
                     
                     <div className="inline-flex items-center gap-2 bg-black/40 backdrop-blur-md border border-yellow-500/30 px-4 py-1.5 rounded-full">
                        <TrophyIcon />
            ...
            {isEditing && (
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    <div className="text-white text-center bg-black/50 p-2 rounded-lg backdrop-blur-sm">
                        <EditIcon />
        ...
        <div className="px-6 pb-8 relative">
            ...
            {isEditing && (
                <div className="relative flex flex-col items-center -mt-16 mb-6 z-10">
                    <div
                        className="relative w-32 h-32 rounded-full border-4 border-gray-800 shadow-xl group bg-gray-700 cursor-pointer hover:scale-105 transition-all duration-300"
            ...
            <div className="grid grid-cols-3 gap-4 mb-8 pt-4">
                <div className="bg-gray-700/50 rounded-xl p-3 text-center border border-gray-700">
                ...
                <div className="bg-gray-700/50 rounded-xl p-3 text-center border border-gray-700 relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-1 opacity-10"><StarIcon /></div>
                    <p className="text-xl sm:text-2xl font-bold text-green-400">{user.points}</p>
                    <p className="text-[10px] sm:text-xs text-gray-400 uppercase font-bold tracking-wide">Pontos</p>
                </div>
            </div>

            {!isEditing && (
                <div className="bg-gray-700/30 rounded-xl p-4 border border-gray-700 text-center mb-8">
                    ...
                    {user.favoriteTeam ? (
                        <div className="flex items-center justify-center gap-3">
                            {user.favoriteTeamLogoUrl ? (
                                <img src={user.favoriteTeamLogoUrl} alt={user.favoriteTeam} className="w-8 h-8 object-contain" />
                            ) : (
            ...
            {isEditing ? (
                <div className="space-y-6 animate-fade-in">
                    <div className="bg-gray-700/30 p-5 rounded-xl border border-gray-700">
                        <h3 className="text-white font-bold mb-4 border-b border-gray-700 pb-2">Dados Pessoais</h3>
                    ...
                    <div className="bg-gray-700/30 p-5 rounded-xl border border-gray-700">
                         <h3 className="text-white font-bold mb-4 border-b border-gray-700 pb-2">Perfil Esportivo</h3>
                    ...
                    <div className="flex gap-4 pt-2">
                        <button onClick={handleCancel} className="flex-1 bg-gradient-to-r from-gray-700 to-gray-600 text-white font-bold py-3 rounded-lg hover:brightness-110 transition-all" disabled={isUploadingAvatar || isUploadingBanner || isSaving}>
                            Cancelar
                        </button>
                        <button onClick={handleSave} className="flex-1 bg-gradient-to-r from-green-600 to-blue-600 text-white font-bold py-3 rounded-lg hover:brightness-110 transition-all flex justify-center items-center" disabled={isUploadingAvatar || isUploadingBanner || isSaving}>
                            {isSaving ? (
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                         {user.sport && user.sport.length > 0 && (
                            <div>
                                ...
                                <div className="flex flex-wrap justify-center gap-2">
                                    {user.sport.map(s => (
                                        <span key={s} className="bg-gray-700 text-green-400 border border-green-500/20 px-3 py-1 rounded-full text-sm font-medium">
                                            {s}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {user.position && user.position.length > 0 && (
                            <div>
                                ...
                                <div className="flex flex-wrap justify-center gap-2">
                                    {user.position.map(p => (
                                        <span key={p} className="bg-gray-700 text-gray-300 border border-gray-600 px-3 py-1 rounded-full text-sm">
                                            {p}
                                        </span>
                                    ))}
                                </div>
                    ...
                    <div className="pt-6 space-y-3 border-t border-gray-700 mt-6">
                        <button onClick={() => setIsEditing(true)} className="w-full bg-gradient-to-r from-gray-700 to-gray-600 text-white font-bold py-3 rounded-lg hover:brightness-110 transition-all flex justify-center items-center gap-2">
                            <EditIcon /> Editar Informações
                        </button>
                        <button onClick={onLogout} className="w-full bg-gradient-to-r from-red-600 to-red-400 text-white font-bold py-3 rounded-lg hover:brightness-110 transition-all">
                            Sair da Conta
                        </button>
                    </div>
                </div>
